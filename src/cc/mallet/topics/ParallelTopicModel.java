...
public class ParallelTopicModel implements Serializable, TopicModel {
    ...
    public static final String ALPHA_SUM = "alphaSum";
    public static final String BETA = "beta";

    public ParallelTopicModel() {
    }

    public void setParams(Map params) {
        if (params.containsKey(NUM_TOPICS)) {
            if (params.containsKey(ALPHA_SUM) && params.containsKey(BETA)) {
                setupPTM(newLabelAlphabet((Integer) params.get(NUM_TOPICS)),
                        (Double) params.get(ALPHA_SUM),
                        (Double) params.get(BETA));
            } else {
                setupPTM(newLabelAlphabet((Integer) params.get(NUM_TOPICS)),
                        0.0 + (Integer) params.get(params.get(NUM_TOPICS)),
                        (Double) params.get(BETA));
            }
        }
        if (params.containsKey(NUM_THREADS)) {
            setNumThreads((Integer) params.get(NUM_THREADS));
        }
        if (params.containsKey(NUM_ITERATIONS)) {
            setNumIterations((Integer) params.get(NUM_ITERATIONS));
        }
    ...
    public List<TopicModelResult> getDocumentTopics() {
        ...
        for (int doc = 0; doc < data.size(); doc++) {
            ...
            result.add(new TopicModelResult(doc, data.get(doc).instance.getSource(), sortedTopics.clone()));
    ...
    public ParallelTopicModel(LabelAlphabet topicAlphabet, double alphaSum, double beta) {
        setupPTM(topicAlphabet, alphaSum, beta);
    }
    ...
    public void estimate() throws IOException {
        ...
        Queue<WorkerRunnable> workers = new LinkedList<WorkerRunnable>();
        workers.addAll(Arrays.asList(runnables));
        ...
        for (int iteration = 1; iteration <= numIterations; iteration++) {
            ...
            if (numThreads > 1) {
                ...
                Queue<Future<WorkerRunnable>> futures = new LinkedList<Future<WorkerRunnable>>();
                ...
                for (int thread = 0; thread < numThreads; thread++) {
                    ...
                    futures.add(executor.submit(runnables[thread]));
                    //runnables[thread].run();
                }*/

//                executor.shutdown();

//                System.err.println("waiting on executor termination");
//                while (!executor.isTerminated()){}
//                System.err.println("getting futures");

                Future<WorkerRunnable> future;
                while ((future = futures.poll()) != null) {
                    try {
                        WorkerRunnable workerRunnable;
                        if ((workerRunnable = future.get()) != null) {
                ...
                /*while (!finished) {
                    ...
                    try {
//						Thread.sleep(10);
                        System.err.println("waiting on executor");
...
//						System.err.println("parent stupidly waiting for children.");
